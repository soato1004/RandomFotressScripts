...
namespace RandomFortress.Game
{
    ...
    public class MonsterBase : UnitBase
    {
        ...
        [Header("링크")] [SerializeField] private HpBar hpBar; // 몬스터 HP 오브젝트
        ...
        {
            // GameObject prefab = ResourceManager.Instance.GetPrefab("DamageText");
            // GameObject damageGo = Instantiate(prefab, transform.position, Quaternion.identity);
            GameObject damageGo = ObjectPoolManager.Instance.GetDamageText(transform.position);
            damageGo.transform.SetParent(GameManager.Instance.game.uiEffectParent);
            damageGo.GetComponent<DamageText>().Show(transform, damage);
        }
        ...
        private IEnumerator MonsterUpdate()
        {
            ...
            {
                if (isDestroyed || monsterState == MonsterState.die || GameManager.Instance.isGameOver )
                    yield break;
                ...
                {
                    wayPoint++;
                    if (wayPoint >= player.wayPoints.Length)
                    {
                        Remove();
