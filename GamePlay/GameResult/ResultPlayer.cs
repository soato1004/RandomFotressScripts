    ...
    public class ResultPlayer : MonoBehaviour
    {
        ...
        public void ShowTower(TowerBase[] towers, int stage)
        {
            ...
            foreach(KeyValuePair<int, TowerResultData> data in towerDic)
            {
                ...
                // icon.SetNativeSize();
                //
                // float ratioX = TARGET_WIDTH / icon.sprite.rect.width;
                // float ratioY = TARGET_HEIGHT / icon.sprite.rect.height;
                // float ratio = ratioX > ratioY ? ratioY : ratioX;
                // icon.transform.localScale = new Vector3(ratio, ratio);
                // icon.transform.localPosition = Vector3.zero;
                ScaleImageToFit(icon);
        ...
        void ScaleImageToFit(Image image)
        {
            float imageWidth = image.sprite.rect.width;
            float imageHeight = image.sprite.rect.height;

            // Sprite의 가로 세로 비율 계산
            float widthRatio = TARGET_WIDTH / imageWidth;
            float heightRatio = TARGET_HEIGHT / imageHeight;
